...
class Dia:
    ...
    def _prepare_text_input(self, text: str) -> torch.Tensor:
        ...
        Multilingual tags like [sw] are mapped via LANG2BYTE (msingiai/dia needs sw=8).
        """
        ...
        # Required for Swahili SFT checkpoints trained on stlohrey LANG2BYTE.
        LANG2BYTE = {
            "sw": 8,
            "en": 3,
            "de": 4,
        ...
        for lang, byte_val in LANG2BYTE.items():
            tag = f"[{lang}]".encode("ascii")
            replaced_bytes = replaced_bytes.replace(tag, bytes([byte_val]))
    ...
    def load_audio(self, audio_path: str) -> torch.Tensor:
        # Prefer soundfile: recent torchaudio.load() requires torchcodec (not always installed).
        import soundfile as sf

        data, sr = sf.read(audio_path, dtype="float32", always_2d=True)  # (T, C)
        audio = torch.from_numpy(np.ascontiguousarray(data.T))  # (C, T)
        if audio.shape[0] > 1:
            audio = audio.mean(dim=0, keepdim=True)
        if int(sr) != DEFAULT_SAMPLE_RATE:
            audio = torchaudio.functional.resample(audio, int(sr), DEFAULT_SAMPLE_RATE)
